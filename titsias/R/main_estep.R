...
if(opt$nbiter.mcmc[5]>0) {
		...
		for (u in 1:opt$nbiter.mcmc[2]) {
			for(vk2 in 1:nb.etas) {
				...
				gradfc <- matrix(0L, nrow = length(fpred1), ncol = nb.etas) 
				#				cat('vk2=',vk2,' nrs2=',nrs2,"\n")				
				...
				for (j in 1:nb.etas) {
					psiM2 <- psiM
					psiM2[,j] <- psiM[,j]+psiM[,j]/1000
					fpred1<-structural.model(psiM, Dargs$IdM, Dargs$XM)
					fpred2<-structural.model(psiM2, Dargs$IdM, Dargs$XM)
					for (i in 1:(Dargs$NM)){
						r = which(Dargs$IdM==i)
						gradf[r,j] <- (fpred2[r] - fpred1[r])/(psiM[i,j]/1000)
					}
				}
				...
				for (i in 1:(Dargs$NM)){
					r = which(Dargs$IdM==i)
					temp <- t(gradf[r,])%*%(Dargs$yM[r]-fpred1[r])
			        mean[i,] <- t(temp)%*%omega.eta
				}
		
				etaMc[,vk2]<- mean[,vk2] + matrix(rnorm(Dargs$NM*nrs2), ncol=nrs2)%*%mydiag(varList$domega2[vk2,nrs2],nrow=1)
				...
				for (j in 1:nb.etas) {
					psiM2 <- psiMc
					psiM2[,j] <- psiMc[,j]+psiMc[,j]/1000
					fpred3<-structural.model(psiMc, Dargs$IdM, Dargs$XM)
					fpred4<-structural.model(psiM2, Dargs$IdM, Dargs$XM)
					for (i in 1:(Dargs$NM)){
						r = which(Dargs$IdM==i)
						gradfc[r,j] <- (fpred4[r] - fpred3[r])/(psiM[i,j]/1000)
					}
				}
				...
				for (i in 1:(Dargs$NM)){
					r = which(Dargs$IdM==i)
					tempc <- t(gradfc[r,])%*%(Dargs$yM[r]-fpred3[r])
			        meanc[i,] <- t(tempc)%*%omega.eta
				}
				P<-0.5*rowSums((etaM-meanc)*(etaM-meanc)%*%somega)
				Pc<-0.5*rowSums((etaMc-mean)*(etaMc-mean)%*%somega)
