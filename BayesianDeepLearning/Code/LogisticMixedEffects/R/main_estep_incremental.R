...
if(opt$nbiter.mcmc[5]>0){	
	for (i in 1:(Dargs$NM)){
		ind <- which(Dargs$IdM==i)
		state <- gibbs_sampler(Dargs$yM[ind], as.matrix(Dargs$XM[ind,]), lambda=0.001,b=etaM[i,],B=omega.eta, n_iter_total=iter_mcmc, burn_in=burn)
		etaM[i,] <- state$beta[(iter_mcmc-burn),]
	}
	phiM<-mean.phiM+etaM
}
