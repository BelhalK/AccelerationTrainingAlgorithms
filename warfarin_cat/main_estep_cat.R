...
estep_cat<-function(kiter, Uargs, Dargs, opt, structural.model, mean.phi, varList, DYF, phiM,saemixObject) {
	...
	U.y<-colSums(DYF)
	post <- list(matrix(nrow = max(opt$nbiter.mcmc),ncol = ncol(phiM)))
	for (i in 1:(nrow(phiM))) {
		post[[i]] <- matrix(nrow = max(opt$nbiter.mcmc),ncol = ncol(phiM) )
	}

	post_new <- list(matrix(nrow = max(opt$nbiter.mcmc),ncol = ncol(phiM)))
	for (i in 1:(nrow(phiM))) {
		post_new[[i]] <- matrix(nrow = max(opt$nbiter.mcmc),ncol = ncol(phiM) )
	}
