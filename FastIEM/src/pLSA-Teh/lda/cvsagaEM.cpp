...
CVSAGAEM::CVSAGAEM(Corpus &corpus, Corpus &to_corpus, Corpus &th_corpus,
...
void CVSAGAEM::GetPosterior(double *theta, double *phi, double *prob) {
    double sum = 0;
    for (int k = 0; k < K; k++) {
...
void CVSAGAEM::Theta(double *cdk, double *theta, int N) {
    double Z = 1.0 / (N + alpha_bar);
    for (int k = 0; k < K; k++)
        theta[k] = (cdk[k] + alpha) * Z;
}

void CVSAGAEM::Phi(std::vector<double> &cvk)
{
...
void CVSAGAEM::BatchIteration(std::vector<int> &docs,
