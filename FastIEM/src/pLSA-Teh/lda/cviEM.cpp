...
CVIEM::CVIEM(Corpus &corpus, Corpus &to_corpus, Corpus &th_corpus,
...
void CVIEM::GetPosterior(double *theta, double *phi, double *prob) {
    double sum = 0;
    for (int k = 0; k < K; k++) {
...
void CVIEM::Theta(double *cdk, double *theta, int N) {
    double Z = 1.0 / (N + alpha_bar);
    for (int k = 0; k < K; k++)
        theta[k] = (cdk[k] + alpha) * Z;
}

void CVIEM::Phi(std::vector<double> &cvk)
{
...
void CVIEM::BatchIteration(std::vector<int> &docs,
