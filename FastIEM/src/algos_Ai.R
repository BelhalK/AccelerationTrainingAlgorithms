...
mixt.iem <- function(x, theta0, K,A,B,sigma,omega,id,nbr)
{
  ...
  somega <- solve(omega)
  ssigma <- solve(sigma)
  gamma <- vector("list", length=n)
  const <- vector("list", length=n)
  for (i in 1:n){
     gammaj <- solve(t(B[[i]])%*%ssigma%*%B[[i]]+somega)
     gamma[[i]]<-gammaj
     constj <- gammaj%*%t(B[[i]])%*%ssigma
     const[[i]]<-constj
  }
  for (j in (1:n))
  {  
    tau[j,]<-const[[j]]%*%(x[j,] - A[[j]]%*%theta$mu)
  }
  ...
  for (k in 1:K)
  {
    ...
    theta<-step.M(tau,n,A,B,sigma,x,id)
    theta.est[k+1,] <- c(k,theta$mu)
    i <- i+nbr
  }
  df <- as.data.frame(theta.est)
