...
saemix.options_nest<-list(seed=39546,map=F,fim=F,ll.is=F, nb.chains = 1, nbiter.mcmc = c(0,0,0,0,iter_mcmc,0,0), sigma.val = 0.01)
nest <- saemix_mala(saemix.model,saemix.data,saemix.options_mala)
post_mala <- nest$post_mala
rates_nest <- nest$rates
...
for (sigma in sigmas){
  print(i)
  saemix.options_nest<-list(seed=39546,map=F,fim=F,ll.is=F, nb.chains = 1, nbiter.mcmc = c(0,0,0,iter_mcmc,0,0,0), sigma.val = sigma)
  nest <- saemix_mala(saemix.model,saemix.data,saemix.options_best)
  rates_nest <- nest$rates
  avg[i,1] <- log(sigma)
  avg[i,2] <- log(mean(rates_nest[-(1:burn)]))
  i <- i+1
}
...
index = 4
names(post_nest[[index]])[2]<-paste("ka")
names(post_nest[[index]])[3]<-paste("V")
names(post_nest[[index]])[4]<-paste("Cl")


graphConvMC_twokernels(post_nest[[index]],post_nest[[index]], title="RWM vs nest")
...
graphConvMC_new(final_nest, title="MALA")
