...
plot.S6 <- plot.prediction.intervals(final6[c(1,4,2,3)], 
                                    ...
                                    legend.title = "algos",
                                    colors       = c('#01b7a5', '#c17b01'))
plot.S6 <- plot.S6  + ylab("w3")+ theme(legend.position=c(0.9,0.8))+ theme_bw()
...
var_rwm <- 0
error_rwm <- 0
true_param <- c(8,5,0.625,1,1,0.01,0.7)
for (j in 1:replicate){
  sample_mean_rwm <- sample_mean_rwm + colMeans(final_rwm[(j*K1):(j*(K1+K2)),2:8])
}
sample_mean_rwm = 1/replicate*sample_mean_rwm

for (j in 1:replicate){
  var_rwm <- var_rwm + (final_rwm[(j*(K1+K2)),2:8]-sample_mean_rwm)^2
  error_rwm <- error_rwm + (final_rwm[(j*(K1+K2)),2:8]-true_param)^2
}
