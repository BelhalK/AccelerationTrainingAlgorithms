...
error <- 0
for (j in 1:replicate){
  sample_mean <- sample_mean + final_rwm[(K1+K2),2:7]
}
sample_mean = 1/replicate*sample_mean

for (j in 1:replicate){
  var <- var + (final_rwm[(K1+K2),2:7]-sample_mean)^2
  error <- error + (final_rwm[(K1+K2),2:7]-true_param)^2
}
