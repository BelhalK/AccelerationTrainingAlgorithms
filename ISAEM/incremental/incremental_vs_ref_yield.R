...
saemix.options_incremental<-list(seed=39546,map=F,fim=F,ll.is=F, nb.chains = 1, nbiter.mcmc = c(0,iter_mcmc), nb.replacement=100)


post_rwm<-saemix_incremental(saemix.model,saemix.data,saemix.options_rwm)$post_rwm
post_incremental<-saemix_incremental(saemix.model,saemix.data,saemix.options_incremental)$post_incremental


index = 1
graphConvMC_twokernels(post_incremental[[index]],post_incremental[[index]], title="EM")
graphConvMC_twokernels(post_rwm[[index]],post_rwm[[index]], title="EM")
graphConvMC_twokernels(post_rwm[[index]],post_incremental[[index]], title="EM")
...
final_incremental <- post_incremental[[1]]
for (i in 2:length(post_incremental)) {
  final_incremental <- rbind(final_incremental, post_incremental[[i]])
}


#ALl individual posteriors
graphConvMC_new(final_rwm, title="RWM")
graphConvMC_new(final_incremental, title="VB Linear case")
#first individual posteriors
graphConvMC_new(post_rwm[[1]], title="EM")

graphConvMC_twokernels(final_rwm,final_incremental, title="EM")
