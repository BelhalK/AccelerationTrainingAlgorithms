...
for (j in 1:replicate){
    ...
    writeDatamlx(res2a2, result.file = "/Users/karimimohammedbelhal/Documents/GitHub/saem/warfarin_cat/data/cat_mcstudytry.csv")
  cat_data.saemix<-read.table("/Users/karimimohammedbelhal/Documents/GitHub/saem/warfarin_cat/data/cat_mcstudytry.csv", header=T, sep=",")
  saemix.data<-saemixData(name.data=cat_data.saemix,header=TRUE,sep=" ",na=NA, name.group=c("id"),name.response=c("y"),name.predictors=c("y"), name.X=c("time"))
  saemix.model<-saemixModel(model=cat_data.model,description="cat model",   
  ...
  # theo_mix10 <- cbind(iteration, theo_mix10)
  # ML <- theo_mix10[,2:6]
...
# err_mix10[,2:6] <- error_mix10[,1:5]

# err_rwm_scaled <- err_rwm[rep(seq_len(nrow(err_rwm)), each=10),]
# err_mix_scaled <- err_mix[rep(seq_len(nrow(err_mix)), each=5),]
# err_mix25_scaled <- err_mix25[rep(seq_len(nrow(err_mix25)), each=2),]
# err_rwm_scaled$iteration = 1:(10*(K1+K2+1))
# err_mix_scaled$iteration = 1:(5*(K1+K2+1))
# err_mix25_scaled$iteration = 1:(2*(K1+K2+1))
...
c <- graphConvMC_sec(err_rwm_scaled[0:end,c(1,2,7)],err_mix_scaled[0:end,c(1,2,7)],err_mix25[0:end,c(1,2,7)])
d <- graphConvMC_sed(err_rwm_scaled[0:end,c(1,5,7)],err_mix_scaled[0:end,c(1,5,7)],err_mix25[0:end,c(1,5,7)])

grid.arrange(c,d, ncol=2)

e <- graphConvMC_sec(err_rwm_scaled[0:end,c(1,3,7)],err_mix_scaled[0:end,c(1,3,7)],err_mix25[0:end,c(1,3,7)])
f <- graphConvMC_sed(err_rwm_scaled[0:end,c(1,6,7)],err_mix_scaled[0:end,c(1,6,7)],err_mix25[0:end,c(1,6,7)])
