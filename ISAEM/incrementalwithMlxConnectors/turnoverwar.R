...
project.file <- "turnoverWarfa/warfarinPK_project.mlxtran"
...
warfarin.saemix <- merge(treat ,warfa_data$y1,by="id")
...
saemix.model<-saemixModel(model=model1cpt,description="warfarin",type="structural"
  ,psi0=matrix(c(1,1,10,0.1,1,1,1,1),ncol=8,byrow=TRUE, dimnames=list(NULL, c("Tlag","ka","V","Cl","Imax","IC50","Rin","kout"))),fixed.estim=c(1,1,1,1,1,1,1,1),
...
options<-list(seed=seed0,map=F,fim=F,ll.is=F,save.graphs=FALSE,nb.chains = 1,nbiter.mcmc = c(2,2,2,0),
 ...
 map.range=c(0), nb.replacement=100,sampling='seq',duration=runtime)
