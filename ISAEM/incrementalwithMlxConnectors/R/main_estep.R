...
if(opt$nbiter.mcmc[4]>0 & kiter %in% map_range){
	...
	if(Dargs$type=="structural"){
		...
		colnames(tempsi_map) <- c("id",colnames(varList$omega))
		...
		for (j in 1:nb.etas) {
			...
			colnames(tempsi_map2) <- c("id",colnames(varList$omega))
			fpred2 <- computePredictions(data.frame(tempsi_map2))$Cc
		...
		tempsi_map <- cbind(unique(Dargs$IdM), psi_map)
	    colnames(tempsi_map) <- c("id",colnames(varList$omega))
	    fpred <- computePredictions(data.frame(tempsi_map))$Cc
