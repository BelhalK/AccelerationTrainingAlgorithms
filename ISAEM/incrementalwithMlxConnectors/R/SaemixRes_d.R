...
#' @section Methods:
#'   \describe{
#'     \item{[<-}{\code{signature(x = "SaemixRes_d")}: replace elements of object}
#'     \item{[}{\code{signature(x = "SaemixRes_d")}: access elements of object}
#'     \item{initialize}{\code{signature(.Object = "SaemixRes_d")}: internal function to initialise object, not to be used}
#'     \item{print}{\code{signature(x = "SaemixRes_d")}: prints details about the object (more extensive than show)}
#'     \item{read}{\code{signature(object = "SaemixRes_d")}: internal function, not to be used }
#'     \item{showall}{\code{signature(object = "SaemixRes_d")}: shows all the elements in the object}
#'     \item{show}{\code{signature(object = "SaemixRes_d")}: prints details about the object}
#'     \item{summary}{\code{signature(object = "SaemixRes_d")}: summary of the results. Returns a list with a number of elements extracted from the results ().}
#' 	 }
...
#' @examples
#' methods(class="SaemixRes_d")
#' 
#' showClass("SaemixRes_d")
#' 
#' @keywords classes
...
setMethod(
  f="initialize",
  signature="SaemixRes_d",
  definition= function(.Object,name.fixed,name.random,name.res,fixed.effects, fixed.psi,betaC,betas,omega,cond.mean.phi,cond.var.phi,mean.phi,phi, phi.samp,parpop, allpar,MCOV){
...
#    cat("Dataset",x@name.data,"\n")
    ...
    if(length(x@se.fixed)==0) {
      tab<-cbind(c(x@name.fixed,x@name.res[x@indx.res]), c(x@fixed.effects))
...
setMethod("show","SaemixRes_d",
  function(object) {
#    cat("Nonlinear mixed-effects model fit by the SAEM algorithm\n")
    cat("Fixed effects\n")
    if(length(object@se.fixed)==0) {
      tab<-cbind(c(object@name.fixed,object@name.res[object@indx.res]), c(object@fixed.effects))
      colnames(tab)<-c("Parameter","Estimate")
    } else {
      tab<-cbind(c(object@name.fixed,object@name.res[object@indx.res]), c(object@fixed.effects), c(object@se.fixed,object@se.respar[object@indx.res]))
      tab<-cbind(tab,100*abs(as.double(tab[,3])/as.double(tab[,2])))
      colnames(tab)<-c("Parameter","Estimate","  SE"," CV(%)")
...
#' 
#' @name fitted.saemix
#' @aliases fitted fitted.SaemixRes fitted.SaemixObject
#' 
