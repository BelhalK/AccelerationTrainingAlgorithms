...
if(opt$nbiter.mcmc[6]>0 & kiter %in% map_range) {
	etaMc<-etaM
	propc <- U.eta
	prop <- U.eta
	for(indiv in 1:Dargs$NM){
		obs <- Dargs$yM[Dargs$IdM==indiv]
		# dose <- unique(Dargs$XM[Dargs$IdM==indiv,1])
		dose <- Dargs$XM[Dargs$IdM==indiv,1][1]
		...
		warmup <- 100
		fit <- sampling(stan.model, data = stan_data, iter = opt$nbiter.mcmc[6]+warmup,warmup = warmup,
			chains = 1,algorithm = "NUTS", init = psiM[indiv,]) #can try "HMC", "Fixed_param"
		# browser()
		fit_samples = extract(fit)
		psiM[indiv,] <- tail(fit_samples$beta,1)
	}
}
