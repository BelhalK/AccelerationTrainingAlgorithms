...
geom_boxplot(data=test[test$group=="Truth",],
            aes(x=factor(group), y=value,),fill="yellow")+
labs(fill="Iteration")+
theme_bw()+ylab(expression(paste(ka[i])))+
theme(legend.title = element_text(size=40),
  legend.key.size = unit(5,"line"),
      legend.text = element_text(size=40),
      axis.text=element_text(size=32), 
      ...
      panel.border = element_rect(colour = "black", fill=NA, size=2),
      plot.margin=unit(c(0.1,0.5,0.1,0.1),"cm"))+
xlab("")+ylab(expression(paste(ka[i]))) + geom_hline(yintercept = quantile(truthbox[,variable],0.25), col="purple", linetype="dashed",size=1.5)+
...
#      xlab("Algorithm")+ylab("")
ggsave(save, file="pics_newlayout/boxnew.pdf", width = 900, height = 350, units = "mm")
