setwd("/Users/karimimohammedbelhal/Desktop/novariability/R")
...
setwd("/Users/karimimohammedbelhal/Desktop/novariability/")
...
for (m in 1:replicate){
  ...
  ##### AV
  options_warfa_with<-list(seed=39546,map=F,fim=F,ll.is=T,nbiter.mcmc = c(2,2,2,0,0),nbiter.saemix = c(K1,K2),displayProgress=TRUE,nbiter.burn =0, map.range=c(0),av=1)
  ...
  ##### AV and new kernel
  options_newkernel<-list(seed=39546,map=F,fim=F,ll.is=T,nbiter.mcmc = c(2,2,2,6,0),nbiter.saemix = c(K1,K2),displayProgress=TRUE,nbiter.burn =0,map.range=c(1:3), av=1)
