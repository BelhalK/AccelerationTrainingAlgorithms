...
estep_mcmc<-function(kiter, Uargs, Dargs, opt, structural.model, mean.phi, varList, DYF, phiM) {
	...
	post <- list(as.data.frame(matrix(nrow = opt$nbiter.mcmc,ncol = ncol(phiM)+2)))
	for (i in 1:(nrow(phiM))) {
		post[[i]] <- as.data.frame(matrix(nrow = opt$nbiter.mcmc,ncol = ncol(phiM)+2))
		names(post[[i]])[1] <- "iteration" 
		names(post[[i]])[ncol(post[[i]])] <- "individual"
		post[[i]][,1] <- 1:opt$nbiter.mcmc
