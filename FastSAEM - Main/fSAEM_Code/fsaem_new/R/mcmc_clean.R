  	...
  	if(Dargs$type=="structural"){
		...
		for (j in 1:nb.etas) {
			...
			for (i in 1:(Dargs$NM)){
				...
				gradf[r,j] <- (fpred2[r] - fpred1[r])/(phi_map[i,j]/1000)
			}
		}
		# gradf[,1] <- 1

		# delta <- transphi(phi_map/1000,saemixObject["model"]["transform.par"]) 
		# for (j in 1:nb.etas) {
		# 	psi_map2 <- psi_map
		# 	psi_map2[,j] <- psi_map[,j] + delta[,j]
		# 	fpred1<-structural.model(psi_map, Dargs$IdM, Dargs$XM)
		# 	fpred2<-structural.model(psi_map2, Dargs$IdM, Dargs$XM)
		# 	for (i in 1:(Dargs$NM)){
		# 		r = 1:sum(Dargs$IdM == i)
	#               r = r+sum(as.matrix(gradf[,j]) != 0L)
		# 		gradf[r,j] <- (fpred2[r] - fpred1[r])/delta[,j]
		# 	}
		# }
...
for (m in 1:saemix.options$L_mcmc) {
	if(m%%100==0){
  	...
  	for (u in 1:opt$nbiter.mcmc[4]) {
		...
		for (i in 1:(Dargs$NM)){
			Mi <- matrix(rnorm(nb.etas), ncol=nb.etas)%*%chol.Gamma[[i]]
		...
		if(Dargs$type=="structural"){
			...
			Uc.y<-compute.LLy_d(phiMc,varList$pres,Uargs,Dargs,DYF)
		}
