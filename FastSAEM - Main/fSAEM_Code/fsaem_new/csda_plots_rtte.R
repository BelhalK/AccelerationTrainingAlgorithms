...
# save.image("rtte_csda_goodenough.RData")
...
source('graphplot.R') 
###RTTE
timetoevent.saemix <- read.table("/Users/karimimohammedbelhal/Desktop/ongoing_research/CSDA/CSDA_code/rtte/rtte3se.csv", header=T, sep=",")
...
for (m in 1:replicate){
  ...
  options.new<-list(seed=39546,map=F,fim=F,ll.is=F,nb.chains = 1, nbiter.mcmc = c(2,2,2,6),
    nbiter.sa=0,nbiter.saemix = c(K1,K2),displayProgress=FALSE, map.range=c(1:3),nbiter.burn =0)
...
head(read.table("/Users/karimimohammedbelhal/Desktop/ongoing_research/CSDA/csda_new/rtte3se.csv", header=T, sep=","))
  ...
  options.mix<-list(seed=seed0,map=F,fim=F,ll.is=F,nb.chains = 1, nbiter.mcmc = c(2,2,2,6),nbiter.sa=0,nbiter.saemix = c(K1,K2),displayProgress=FALSE, map.range=c(1:4),nbiter.burn =0)
