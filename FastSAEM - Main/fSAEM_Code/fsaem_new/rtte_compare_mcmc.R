...
setwd("/Users/karimimohammedbelhal/Desktop/research/CSDA/csda_new/R")
...
setwd("/Users/karimimohammedbelhal/Desktop/research/CSDA/csda_new")
source('graphplot.R') 
load("rtte_mcmc_conv.RData")
###RTTE
timetoevent.saemix <- read.table("/Users/karimimohammedbelhal/Desktop/research/CSDA/csda_new/data/rtte_data.csv", header=T, sep=",")
 ...
 G <- (ncol(df)-2)/3
  ...
  for (j in (2:(ncol(df)-1)))
  {
    if (j<3){
      ...
      xlab("")+ theme_bw() +ylab(expression(paste(hat(q)[alpha](lambda[i]))))
      grafj <- grafj + theme_bw() + theme(legend.position = "none", axis.text=element_text(size=34), 
                 axis.title=element_text(size=40),
                   panel.border = element_rect(colour = "black", fill=NA, size=2))
    }else{
      grafj <- ggplot(df)+geom_line(aes_string(df[,1],df[,j],by=df[,ncol(df)]),colour="blue",size=0.8) +geom_line(aes_string(df2[,1],df2[,j],by=df2[,ncol(df2)]),colour="red",linetype = 1,size=0.8)+
      xlab("")+ theme_bw() +ylab(expression(paste(hat(q)[alpha](beta[i]))))
      grafj <- grafj + theme_bw() + theme(legend.position = "none", axis.text=element_text(size=34), 
                 axis.title=element_text(size=40),
                   panel.border = element_rect(colour = "black", fill=NA, size=2))
    }
...
ggsave(save, file="quantttenotlog.pdf", width = 900, height = 225, units = "mm")
