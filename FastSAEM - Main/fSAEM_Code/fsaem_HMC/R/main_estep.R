...
### IF HMC PROPOSAL###### IF HMC PROPOSAL###### IF HMC PROPOSAL###### IF HMC PROPOSAL###
if(opt$nbiter.mcmc[5]>0 & kiter %in% map_range){
	for (i in 1:Dargs$NM){
		...
		design[,1] <- Dargs$XM[Dargs$IdM==i,1][1]
		...
		stan_data <- list(N = length(obs),concentration = obs, dose = Dargs$XM[Dargs$IdM==i,1][1]
						,time = design[-1,2],
		...
		fit <- sampling(stan.model, data = stan_data, iter = 10,chains=1, init = 'random',algorithm = "NUTS") #can try "HMC", "Fixed_param"
		# fit <- sampling(stan.model, data = stan_data, iter = 6,algorithm = "NUTS") #can try "HMC", "Fixed_param"

		fit_samples = extract(fit)
		psiMstan <- tail(fit_samples$beta,1)
