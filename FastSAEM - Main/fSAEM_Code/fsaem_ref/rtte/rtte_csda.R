...
saemix.model<-saemixModel(model=timetoevent.model,description="time model",   
  psi0=matrix(c(4,1),ncol=2,byrow=TRUE,dimnames=list(NULL,   
  c("lambda","beta"))), 
  transform.par=c(1,1),covariance.model=matrix(c(2,0,0,2),ncol=2, 
  byrow=TRUE))
...
writeDatamlx(res, result.file = "/Users/karimimohammedbelhal/Desktop/ongoing_research/CSDA/CSDA_code_ref/rtte/rtte3se.csv")
head(read.table("/Users/karimimohammedbelhal/Desktop/ongoing_research/CSDA/CSDA_code_ref/rtte/rtte3se.csv", header=T, sep=","))
  timetoevent.saemix <- read.table("/Users/karimimohammedbelhal/Desktop/ongoing_research/CSDA/CSDA_code_ref/rtte/rtte3se.csv", header=T, sep=",")
  timetoevent.saemix <- timetoevent.saemix[timetoevent.saemix$ytype==2,]
  saemix.data<-saemixData(name.data=timetoevent.saemix,header=TRUE,sep=" ",na=NA, name.group=c("id"),name.response=c("y"),name.predictors=c("time","y"), name.X=c("time"))
  print(j)
