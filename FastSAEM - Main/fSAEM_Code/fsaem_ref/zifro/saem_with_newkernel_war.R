...
graphConvMC_diff(final_rwm[,c(1,2,9)],final_mix[,c(1,2,9)], title="RWM")
...
err_mix[2,] = err_rwm[2,]
graphConvMC_diff(err_rwm[-1,],err_mix[-1,], title="RWM")
...
# for (j in 1:replicate){
#   print(j)
#   options.new<-list(seed=j*seed0,map=F,fim=F,ll.is=T,nb.chains = 1, nbiter.mcmc = c(0,0,0,6,0),nbiter.saemix = c(K1,K2),map.range=c(1:10),nbiter.burn =0)
#   theo_mix<-data.frame(saemix_new_mix(saemix.model,saemix.data,options.new))
#   theo_mix <- cbind(iterations, theo_mix)
