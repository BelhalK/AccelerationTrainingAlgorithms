...
### IF VI PROPOSAL###### IF VI PROPOSAL###### IF VI PROPOSAL###### IF VI PROPOSAL###
  	
 if (typeof(saemix.options$modelstan)=="S4"){
  	for (i in 1:Dargs$NM){
		...
		passed = FALSE
		while (passed == FALSE){
			fit <- tryCatch(vb(stan.model, data = stan_data, iter = 100000), error= function(e){NULL})
			passed <- !is.null(fit)
		}	

		fit_samples = extract(fit)
		psiMstan <- tail(fit_samples$beta,10000)
