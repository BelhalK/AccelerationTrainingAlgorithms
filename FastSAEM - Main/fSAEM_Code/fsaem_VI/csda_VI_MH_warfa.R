...
load("RData/warfa_visaem.RData")
# save.image("RData/warfa_visaem.RData")

# setwd("/Users/karimimohammedbelhal/Desktop/package_contrib/saemixB/R")
setwd("/Users/karimimohammedbelhal/Desktop/research/CSDA/csda_newVI/R")
...
setwd("/Users/karimimohammedbelhal/Desktop/research/CSDA/csda_newVI")
source('plotfunc/graphplot.R') 


warfa_data <- read.table("/Users/karimimohammedbelhal/Desktop/research/CSDA/csda_newVI/data/warfarin_data.txt", header=T)
...
for (m in 1:replicate){
  print(m)
  l = list(c(3,5,1,0,0,0),c(3.8,12,0.8,0,0,0),c(1.2,2,1.2,0,0,0),c(1.4,6.6,1.4,0,0,0))
  ...
  options.new<-list(seed=m*seed1,map=F,fim=F,ll.is=T,nb.chains = 1, nbiter.mcmc = c(2,2,2,2,0),nbiter.sa=0,nbiter.saemix = c(K1,K2),map.range=c(1:4),nbiter.burn =0, modelstan = NULL)
...
for (m in 1:replicate){
  ...
  options.vi<-list(seed=m*seed1,map=F,fim=F,ll.is=T,nb.chains = 1, nbiter.mcmc = c(2,2,2,2,0),nbiter.sa=0,nbiter.saemix = c(K1,K2),map.range=c(1:4),nbiter.burn =0, modelstan = modelstan)
  theo_vi_ref<-data.frame(saemix(saemix.model,saemix.data,options.vi)$par)
  theo_vi <- cbind(iterations, theo_vi_ref)
  theo_vi[,5:7] <- sqrt(theo_vi[,5:7])
  theo_vi['individual'] <- m
  final_vi <- rbind(final_vi,theo_vi[-1,])
}
...
convpop <- graphConvMC_diffvi2(final_rwm[,c(1,3,9)],final_mix[,c(1,3,9)],final_vi[,c(1,3,9)])
convvar <- graphConvMC_diffvi1(final_rwm[,c(1,6,9)],final_mix[,c(1,6,9)],final_vi[,c(1,6,9)])
save <- grid.arrange(convpop,convvar, ncol=2)


convpop <- graphConvMC_diffvi2(final_rwm[,c(1,2,9)],final_mix[,c(1,2,9)],final_vi[,c(1,2,9)])
convvar <- graphConvMC_diffvi1(final_rwm[,c(1,5,9)],final_mix[,c(1,5,9)],final_vi[,c(1,5,9)])
save <- grid.arrange(convpop,convvar, ncol=2)

convpop <- graphConvMC_diffvi2(final_rwm[,c(1,4,9)],final_mix[,c(1,4,9)],final_vi[,c(1,4,9)])
convvar <- graphConvMC_diffvi1(final_rwm[,c(1,7,9)],final_mix[,c(1,7,9)],final_vi[,c(1,7,9)])
save <- grid.arrange(convpop,convvar, ncol=2)
