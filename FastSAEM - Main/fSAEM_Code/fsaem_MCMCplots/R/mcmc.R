...
mcmc<-function(model,data,control=list()) {
	...
	#Store the states for all individuals
	...
	if(opt$nbiter.mcmc[4]>0) {
		...
		l<-c()
			for (u in 1:opt$nbiter.mcmc[5]) {
				...
				for (i in 1:Dargs$NM){
					etaMc[i,] <- etaM[i,] + sigma*gradU[i,] + sqrt(2*sigma*adap[i])*Z[i,]
				}
				...
				for (i in 1:(Dargs$NM)){
					propc[i,] <- ((etaMc[i,]-etaM[i,] - sigma*gradU[i,])/sqrt(2*sigma))^2
					prop[i,] <- ((etaM[i,]-etaMc[i,] - sigma*gradUc[i,])/sqrt(2*sigma))^2
				}
