...
for (m in 1:replicate){
  print(m)
  l = list(c(5,0.4,0.2,0.1,1,1),c(7,1.4,0.1,0.3,1,1),c(9,0.8,1,0.05,1,1))
  ...
  options<-list(seed=seed0/m,map=F,fim=F,ll.is=F,nb.chains = 1,
   nbiter.sa=0,nbiter.mcmc = c(2,2,2,0,0,0),nbiter.saemix = c(K1,K2),map.range=c(0),nbiter.burn =0)
  ...
  options.new<-list(seed=seed0/m,map=F,fim=F,ll.is=F,nb.chains = 1,
   nbiter.sa=0,nbiter.mcmc = c(2,2,2,2,0,0),nbiter.saemix = c(K1,K2),map.range=c(1:K1),nbiter.burn =0)
...
convpop <- graphConvMC_diffpk1(final_rwm[,c(1,4,15)],final_mix[,c(1,4,15)])
convvar <- graphConvMC_diffpk1(final_rwm[,c(1,10,15)],final_mix[,c(1,10,15)])
