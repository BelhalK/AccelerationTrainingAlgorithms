...
step.S_isaem2<-function(x,theta,M)
{
  n<-length(x)
  G<-length(theta$p)
  Z<-array(NA,c(n,G,M))
...
step.SAdecay <-function(x,Z,Z_new,s.old,gamma,k)
{
  S<-compute.stat(x,Z)
  S$s1 <- compute.stat(x,Z_new)$s1+ 1/k*S$s1
  S$s2 <- compute.stat(x,Z_new)$s2+ 1/k*S$s2
  S$s3 <- compute.stat(x,Z_new)$s3+ 1/k*S$s3
  s11<-s.old$s1+gamma*(S$s1-s.old$s1)
  s12<-s.old$s2+gamma*(S$s2-s.old$s2)
  s13<-s.old$s3+gamma*(S$s3-s.old$s3)
  s.new<-list(s1=s11,s2=s12,s3=s13)
...
step.SA2 <-function(x,Z,Z_new,s.old,gamma)
{
  S<-compute.stat(x,Z)
  S$s1 <- compute.stat(x,Z_new)$s1+ (n-nb_r)/n*S$s1
  S$s2 <- compute.stat(x,Z_new)$s2+ (n-nb_r)/n*S$s2
  S$s3 <- compute.stat(x,Z_new)$s3+ (n-nb_r)/n*S$s3
  s11<-s.old$s1+gamma*(S$s1-s.old$s1)
  s12<-s.old$s2+gamma*(S$s2-s.old$s2)
  s13<-s.old$s3+gamma*(S$s3-s.old$s3)
  s.new<-list(s1=s11,s2=s12,s3=s13)
