...
#' 
#' @export saemix
saemix_incremental<-function(model,data,control=list()) {
...
# structural model, check nb of parameters
  ...
  xmcmc<-estep_incremental(1, Uargs, Dargs, opt, structural.model, mean.phi, varList, DYF, phiM, saemixObject)
  ...
  post_incremental<-xmcmc$post_incremental

  return(list(post_rwm = post_rwm,post_incremental = post_incremental))
