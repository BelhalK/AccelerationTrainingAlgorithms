...
\veryHuge \color{Navy} \textbf{Exploitation de données pour l'optimisation de trajectoires aériennes} \color{Black}\\[2cm] % Title
%\Huge\textit{Identification de système dynamique}\\[2cm] % Subtitle
\huge \textbf{Cédric Rommel$^{1,2,3}$, F. Bonnans$^{1,2}$, P. Martinon$^{1,2}$, B. Gregorutti$^3$}\\[0.5cm] % Author(s)
\huge INRIA$^1$, CMAP École Polytechnique$^2$, Safety Line$^3$\\[0.4cm] % University/organization
