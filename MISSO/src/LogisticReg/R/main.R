...
# perm <- replicate(saemix.options$nbiter.tot,sample(1:Dargs$NM,Dargs$NM))
# l <- as.vector(as.matrix(perm[,]))
# m <- matrix(0,ncol=3)
...
# SAEM convergence plots
    ...
    if(saemix.options$displayProgress)    
      try(convplot.infit(parpop,saemix.options$nbiter.saemix[1],niter=(kiter-2)))
  }
  ...
  mean <- xmcmc$mean
  cov <- xmcmc$cov
  finalcov <- cov - t(mean)%*%mean
  
  varList$omega<- finalcov
  for(i in 1:Dargs$NM) {
    mean.phi[i,]<-mean  
  }

  # m <- m + 1/kiter*(mean -m)
  # c <- c + 1/kiter*(finalcov -c)
 theta<-c(mean,diag(finalcov))
  # theta<-c(m,diag(c))
