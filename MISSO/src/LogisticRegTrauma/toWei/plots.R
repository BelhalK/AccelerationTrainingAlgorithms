...
dfpriormean <- data.frame(x,saem.ep,mcem.ep,imcem.ep, imcem10.ep, imcem50.ep)

ggplot(data=dfpriormean)+
  geom_line(mapping=aes(y=saem.ep,x= x,color="saem"),size=0.5 ) +
  geom_line(mapping=aes(y=mcem.ep,x= x,color="mcem"),size=0.5) +
  geom_line(mapping=aes(y=imcem.ep,x= x,color="imcem"),size=0.5) +
...
write.csv(dfpriormean, file = "notebooks/priormean.csv")




#### Logistic parameters beta
...
# df <- dataa.frame(x,saem.ep,mcem.ep,imcem.ep)
...
dflogit <- data.frame(x,saem.ep,mcem.ep,imcem.ep, imcem10.ep, imcem50.ep)
...
# write.csv(dflogit, file = "notebooks/logisticparam.csv")
