...
estep_check<-function(kiter, Uargs, Dargs, opt, structural.model, mean.phi, varList, DYF, phiM) {
	...
	post <- list(as.data.frame(matrix(nrow = max(opt$nbiter.mcmc),ncol = ncol(phiM)+2)))
	for (i in 1:(nrow(phiM))) {
		post[[i]] <- as.data.frame(matrix(nrow = max(opt$nbiter.mcmc),ncol = ncol(phiM)+2))
		names(post[[i]])[1] <- "iteration" 
		names(post[[i]])[ncol(post[[i]])] <- "individual"
		post[[i]][,1] <- 1:max(opt$nbiter.mcmc)
		post[[i]][,ncol(post[[i]])] <- i
	}
	...
	if(opt$nbiter.mcmc[2]>0) {
		...
		for (u in 1:opt$nbiter.mcmc[2]) {
			for(vk2 in 1:nb.etas) {
				...
				etaM[ind,]<-etaMc[ind,]
				for (i in 1:(nrow(phiM))) {
					post[[i]][u,2:(ncol(post[[i]]) - 1)] <- etaM[i,]
				}
