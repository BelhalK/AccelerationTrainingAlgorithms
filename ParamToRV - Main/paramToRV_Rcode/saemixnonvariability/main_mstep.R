...
mstep<-function(kiter, Uargs, Dargs, opt, structural.model, DYF, phiM, varList, phi, betas, suffStat,saemixObject) {
	...
	if (opt$flag.fmin && kiter>=opt$nbiter.sa) {
		...
		if(Dargs$type=="structural"){
			# logpy<-function(b0){
			# 	compute.Uy_c(b0,phiM,varList$pres,Uargs,Dargs,DYF)
			# }
		 #    cma <- cmaNew()
			...
			# beta0<-optim(par=betainit,fn=compute.Uy_c,phiM=phiM,pres=varList$pres,args=Uargs,Dargs=Dargs,DYF=DYF,control=list(maxit=opt$maxim.maxiter))$par
